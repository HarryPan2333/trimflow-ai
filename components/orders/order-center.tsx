  ...
  return <div className="order-workspace">
    ...
    <Card className="order-list-panel">
      ...
      <div className="order-table-scroll"><table className="order-table"><thead><tr><th>PO</th><th>{t("common.client")} · {t("common.project")}</th><th>{t("common.product")}</th><th>{t("order.amount")}</th><th>{t("order.poDate")}</th><th>{t("order.requiredDelivery")}</th><th>{t("common.currentStage")}</th><th>{t("order.health")}</th><th>{t("common.owner")}</th><th>{t("order.nextMilestone")}</th><th>{t("common.nextAction")}</th></tr></thead><tbody>{rows.map((context) => {
        ...
        return <tr key={context.order.id}><td><button className="order-id-link" onClick={() => onOpenOrder(context.order.id)}>{context.order.poNumber}<small>{t("order.openExecution")} →</small></button></td><td><strong>{text(context.client?.name ?? t("order.anonymousClient"))}</strong><small>{context.project?.code} · {text(context.project?.name ?? "")}</small></td><td>{context.lines.map((item) => text(item.configurationSnapshot?.productName ?? item.product)).join(" · ") || text(context.quotation?.product || t("common.notConfirmed"))}</td><td><strong>{formatOrderValue(context.order.amount, context.order.currency)}</strong></td><td>{formatDate(context.order.poDate)}</td><td>{context.order.deliveryDate ? formatDate(context.order.deliveryDate) : t("common.notConfirmed")}</td><td><Badge>{label(context.order.currentStage)}</Badge></td><td><span className={`order-health-label health-${health.status.toLowerCase().replaceAll(" ", "-")}`}>{label(health.status)}</span><small>{text(health.reason)}</small></td><td>{text(context.order.owner)}</td><td>{milestone ? <><strong>{text(milestone.label)}</strong><small>{formatDate(milestone.targetDate)}</small></> : t("common.notConfirmed")}</td><td><span className="order-next-cell">{text(context.project?.next ?? t("order.nextMilestoneFallback"))}</span></td></tr>;
